...
func createStoreAndDomain(config config.Config) *tikv.RawKVClient {
	fullPath := fmt.Sprintf("%s://%s", config.Store, config.Path)
	fmt.Print(fullPath)
	var err error
...
func NewManager(cfg *config.Config) *Manager {
	...
	client := createStoreAndDomain(*cfg)
	DB := memdb.NewMemDb(client)
